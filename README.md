...
Структуру `OKR-<quarter>.md` и `экватор-<quarter>.md` проверяет скрипт, не
самоотчёт модели. Тип документа он определяет сам (путь — относительно корня
установки, куда `install.sh` синкнул навыки; обычно `.claude/skills/...`,
зависит от выбора при установке):

```bash
python3 <корень установки>/skills/okr-equator/scripts/okr-lint.py .okr/<quarter>/OKR-<quarter>.md
```
...
Ненулевой код выхода — документ не по шаблону. Для экватора это потерянный
обязательный раздел, PBV вне диапазона, недопустимый статус. Для OKR — не
собрана одна из обязательных таблиц, показатель точки А без проверяемого
источника, KR без пары в таблице дельты или без блока приёмки в Части 2,
пустая колонка «Где проверяю», процессная формулировка KR, длинное тире.
`/okr-draft` и `/okr-equator` прогоняют линтер перед записью файла и не
сохраняют документ с ошибками. Скрипт лежит в `skills/okr-equator/`, но
проверяет оба документа — отдельного навыка под линтер нет.
